...
值得注意的是群体结构的结果与原文（下图，原文Fig.1 c）有一定差异。经过与作者的讨论得知，作者实际上另有一份对野生稻单独进行变异检测的VCF文件，而不是先野生稻＋栽培稻联合变异检测，然后提取野生稻的SNP。很遗憾作者不愿提供这份单独的野生稻数据集。
...
很明显，使用文章提供的数据所得到的结果不能支持这一结论，*Ruf*和*Niv*之间的差异仍然是最主要的。
...
栽培稻没有形成单系群，而是与部分野生稻混合成簇。具体来说，籼稻（*indica*、*aus*）与*Niv2*形成一个簇，而3个粳稻群体（*aromatic*、温带粳稻、热带粳稻）与华南的一个*O . rufipogon*群体（*Ruf1a*）聚类成一个簇，而印度北部的*Ruf1b*不直接参与水稻驯化。
...
在作者的文章中PSR的筛选是通过自编Perl脚本实现的。相比Perl，Python语言具有更强的可维护性和通用性，因此我全部用Python重写了有关代码，经测试能够输出完全相同的结果。需要指出的是，作者的代码存在不少小问题，例如格式错误等。作者设计的对文本文件建立索引然后根据索引“随机访问”的特性也比较鸡肋，因为无论如何都要滑窗法遍历整个基因组，没有必要随机访问。此外，对于群体遗传统计量的计算，直接调用vcftools等工具或调用scikit-allel等模块可能比从头手写代码效率更高。不过无论如何，模仿和改编作者的代码也是一次很好的学习巩固Python编程和群体遗传基本理论的机会。

滑窗统计的 $ROD_{Wild/Cultiva}$ 和 $F_{ST}$ 分布如下，虚线为阈值线：

![image](figs/fst+rod.png)

在实操中我注意到一个问题，即便已经剔除了离群值， $ROD$ 和 $F_{ST}$ （尤其是 $ROD$ ）仍然均不是正态分布的，且有非常明显的右偏，更加接近泊松分布的形状。在这种情况下选择对数据进行Z变换并按照 $\mu+2\sigma$ 为阈值，恐怕会低估野生稻和栽培稻之间的遗传分化。使用更加合理的理论分布有可能改善分析的结果。
...
结果表明，大部分（80%）的SOR起源于*Ruf1*，但是也有14%的SOR起源于*Niv1/2*，作者（结合选择清除基因的分析）认为这一结果表明了水稻驯化的双重起源，即*Ruf*和*Niv*均贡献了驯化相关基因。
...
作者挑选了部分单起源选择清除基因进行了单倍型分析。具体来说，作者节选了36个功能已知，在籼稻和粳稻中均受到选择，且属于单起源的选择清除基因进行分析，详见原文补充表S16。作者称为了结果的准确，只挑选了891个在Admixture中祖先成分大于0.8的材料进行分析。之后先提取每个基因外加上下游各200kb序列，只保留双等位基因位点，MAF>=0.05，缺失率<=0.1，然后截取上下游1kb范围，过滤掉位点缺失比例高于20%的样本。剩下的位点使用Beagle进行基因型填充。然后进行单倍型网络分析。

然而，考察作者编写的Perl脚本可以发现，作者在提取、过滤SNP，甚至基因型填充时使用的样本集都是包含三个次要品种（共988份材料）的，只有在最后统计单倍型频率分布时才最终保留了891份材料。这样的操作显然会导致MAF计算不准确。
